...
int
xqd_notify (xlator_t *this, int32_t event, void *data, ...)
{
        switch (event) {
        case GF_EVENT_PARENT_UP:
                xquotad_aggregator_init (this);
        }
...
int32_t
xqd_lookup_cbk (call_frame_t *frame, void *cookie, xlator_t *this,
                int32_t op_ret, int32_t op_errno, inode_t *inode,
                struct iatt *buf, dict_t *xdata, struct iatt *postparent)
{
        xquotad_aggregator_lookup_cbk_t  lookup_cbk = NULL;
...
xlator_t *
xqd_find_subvol (xlator_t *this, char *volume_uuid)
{
                     ...
                     dict_t *xdata, xquotad_aggregator_lookup_cbk_t lookup_cbk)
{
        ...
        ret = dict_set_int8 (xdata, XQUOTA_READ_ONLY_KEY, 1);
        if (ret < 0) {
        ...
        subvol = xqd_find_subvol (this, volume_uuid);
        if (subvol == NULL) {
                op_errno = EINVAL;
                goto out;
        }

        STACK_WIND_COOKIE (frame, xqd_lookup_cbk, lookup_cbk, subvol,
...
int
xqd_reconfigure (xlator_t *this, dict_t *options)
{
        /* As of now xquotad is restarted upon alteration of volfile */
        return 0;
}

void
xqd_fini (xlator_t *this)
{
...
xqd_init (xlator_t *this)
{
        ...
        if (NULL == this->children) {
                gf_log (this->name, GF_LOG_ERROR,
                        "FATAL: xquota (%s) not configured for min of 1 child",
                        this->name);
                ret = -1;
                goto err;
        }
