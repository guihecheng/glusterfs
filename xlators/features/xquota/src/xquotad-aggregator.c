...
struct rpcsvc_program xquotad_aggregator_prog;

struct iobuf *
xquotad_serialize_reply (rpcsvc_request_t *req, void *arg, struct iovec *outmsg,
                         xdrproc_t xdrproc)
{
...
int
xquotad_aggregator_submit_reply (call_frame_t *frame, rpcsvc_request_t *req,
                                 ...
                                 xdrproc_t xdrproc)
{
        ...
        iob = xquotad_serialize_reply (req, arg, &rsp, xdrproc);
        if (!iob) {
                gf_msg ("", GF_LOG_ERROR, 0, XQ_MSG_DICT_SERIALIZE_FAIL,
...
ret:
        if (state) {
                xquotad_aggregator_free_state (state);
        }
...
int
xquotad_aggregator_getlimit_cbk (xlator_t *this, call_frame_t *frame,
                                 void *lookup_rsp)
{
...
reply:
        xquotad_aggregator_submit_reply (frame, frame->local, (void*)&cli_rsp, NULL, 0,
...
int
xquotad_aggregator_getlimit (rpcsvc_request_t *req)
{
        ...
        GF_VALIDATE_OR_GOTO ("xquotad-aggregator", req, err);
        ...
        if (ret < 0)  {
                ...
                gf_msg ("this->name", GF_LOG_ERROR, 0, XQ_MSG_XDR_DECODE_ERROR,
                        "xdr decoding error");
                req->rpc_err = GARBAGE_ARGS;
                goto err;
        }
        ...
        frame = xquotad_aggregator_get_frame_from_req (req);
        if (frame == NULL) {
        ...
        ret = dict_set_int32 (state->xdata, VIRTUAL_XQUOTA_USAGE_META_KEY, 42);
        if (ret)
                goto err;
        ...
        ret = xqd_nameless_lookup (this, frame, &args, state->xdata,
                                   xquotad_aggregator_getlimit_cbk);
        if (ret) {
...
errx:
        ...
        xquotad_aggregator_getlimit_cbk (this, frame, &cli_rsp);
        if (dict)
                dict_unref (dict);
...
int
xquotad_aggregator_lookup_cbk (xlator_t *this, call_frame_t *frame,
                              void *rsp)
{
        xquotad_aggregator_submit_reply (frame, frame->local, rsp, NULL, 0, NULL,
...
int
xquotad_aggregator_lookup (rpcsvc_request_t *req)
{
        call_frame_t               *frame = NULL;
        gfs3_lookup_req             args  = {{0,},};
        int                         ret   = -1, op_errno = 0;
...
err:
        ...
        xquotad_aggregator_lookup_cbk (this, frame, &rsp);
        return ret;
}

int
xquotad_aggregator_rpc_notify (rpcsvc_t *rpc, void *xl, rpcsvc_event_t event,
                               void *data)
{
        if (!xl || !data) {
...
xquotad_aggregator_init (xlator_t *this)
{
        ...
        if (ret < 1) {
                gf_msg (this->name, GF_LOG_WARNING, 0,
                        XQ_MSG_RPCSVC_LISTENER_CREATION_FAILED,
                        "creation of listener failed");
                ret = -1;
                goto out;
        }

        priv->xquotad_aggregator = &xquotad_aggregator_prog;
        xquotad_aggregator_prog.options = this->options;

        ret = rpcsvc_program_register (priv->rpcsvc, &xquotad_aggregator_prog);
        if (ret) {
                gf_msg (this->name, GF_LOG_WARNING, 0,
                        XQ_MSG_RPCSVC_REGISTER_FAILED,
                        "registration of program (name:%s, prognum:%d, "
                        "progver:%d) failed", xquotad_aggregator_prog.progname,
                        xquotad_aggregator_prog.prognum,
                        xquotad_aggregator_prog.progver);
                goto out;
        }
...
rpcsvc_actor_t xquotad_aggregator_actors[GF_XAGGREGATOR_MAXVALUE] = {
        [GF_XAGGREGATOR_NULL]     = {"NULL", GF_XAGGREGATOR_NULL, NULL, NULL, 0,
                                    DRC_NA},
        [GF_XAGGREGATOR_LOOKUP]   = {"LOOKUP", GF_XAGGREGATOR_NULL,
                                    xquotad_aggregator_lookup, NULL, 0, DRC_NA},
        [GF_XAGGREGATOR_GETLIMIT] = {"GETLIMIT", GF_XAGGREGATOR_GETLIMIT,
                                    xquotad_aggregator_getlimit, NULL, 0, DRC_NA},
};
...
struct rpcsvc_program xquotad_aggregator_prog = {
        ...
        .numactors = GF_XAGGREGATOR_MAXVALUE,
        .actors    = xquotad_aggregator_actors
};
