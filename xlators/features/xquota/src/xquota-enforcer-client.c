...
extern struct rpc_clnt_program xquota_enforcer_clnt;
...
int
xquota_enforcer_submit_request (void *req, call_frame_t *frame,
...
int
xquota_enforcer_lookup_cbk (struct rpc_req *req, struct iovec *iov,
...
out:
        ...
        /* We need to retry connecting to xquotad on ENOTCONN error.
         * Suppose if there are two volumes vol1 and vol2,
         * and xquota is enabled and limit is set on vol1.
         * Now if IO is happening on vol1 and xquota is enabled/disabled
         * on vol2, xquotad gets restarted and client will receive
         * ENOTCONN in the IO path of vol1
         */
        if (rsp.op_ret == -1 && rsp.op_errno == ENOTCONN) {
                if (local->xquotad_conn_retry >= 12) {
                        priv->xquotad_conn_status = 1;
                        gf_log (this->name, GF_LOG_WARNING, "failed to connect "
                                "to xquotad after retry count %d)",
                                local->xquotad_conn_retry);
                } else {
                        local->xquotad_conn_retry++;
                }

                if (priv->xquotad_conn_status == 0) {
                        /* retry connecting after 5secs for 12 retries
                         * (up to 60sec).
                         */
                        gf_log (this->name, GF_LOG_DEBUG, "retry connecting to "
                                "xquotad (retry count %d)",
                                local->xquotad_conn_retry);

                        retry_delay.tv_sec = 5;
                        retry_delay.tv_nsec = 0;
                        timer = gf_timer_call_after (this->ctx, retry_delay,
                                                     _xquota_enforcer_lookup,
                                                     (void *) frame);
                        if (timer == NULL) {
                                gf_log (this->name, GF_LOG_WARNING, "failed to "
                                        "set xquota_enforcer_lookup with timer");
                        } else {
                                goto clean;
                        }
                }
        } else {
                priv->xquotad_conn_status = 0;
        }
        ...
        } else if (local->xquotad_conn_retry) {
                gf_log (this->name, GF_LOG_DEBUG, "connected to xquotad after "
                        "retry count %d", local->xquotad_conn_retry);
        }
...
_xquota_enforcer_lookup (void *data)
{
        ...
        ret = xquota_enforcer_submit_request (&req, frame,
                                             priv->xquota_enforcer,
                                             GF_XAGGREGATOR_LOOKUP,
                                             NULL, this,
                                             xquota_enforcer_lookup_cbk,
                                             (xdrproc_t)xdr_gfs3_lookup_req);

        if (ret) {
                gf_msg (this->name, GF_LOG_WARNING, 0,
			XQ_MSG_RPC_SUBMIT_FAILED, "Couldn't send the request to "
...
int
xquota_enforcer_notify (struct rpc_clnt *rpc, void *mydata,
...
int
xquota_enforcer_blocking_connect (rpc_clnt_t *rpc)
{
        dict_t *options = NULL;
        int     ret     = -1;
...
//Returns a started rpc_clnt. Creates a new rpc_clnt if xquota_priv doesn't have
//one already
struct rpc_clnt *
xquota_enforcer_init (xlator_t *this, dict_t *options)
{
        ...
        priv->xquota_enforcer = &xquota_enforcer_clnt;
        ...
        ret = rpc_clnt_register_notify (rpc, xquota_enforcer_notify, this);
        if (ret) {
                gf_msg ("xquota", GF_LOG_ERROR, 0,
                        XQ_MSG_RPCCLNT_REGISTER_NOTIFY_FAILED,
                        "failed to register notify");
                goto out;
        }

        ret = xquota_enforcer_blocking_connect (rpc);
        if (ret)
                goto out;
...
struct rpc_clnt_procedure xquota_enforcer_actors[GF_XAGGREGATOR_MAXVALUE] = {
        [GF_XAGGREGATOR_NULL]     = {"NULL", NULL},
        [GF_XAGGREGATOR_LOOKUP]   = {"LOOKUP", NULL},
};

struct rpc_clnt_program xquota_enforcer_clnt = {
        .progname  = "XQuota enforcer",
        .prognum   = GLUSTER_XAGGREGATOR_PROGRAM,
        .progver   = GLUSTER_XAGGREGATOR_VERSION,
        .numproc   = GF_XAGGREGATOR_MAXVALUE,
        .proctable = xquota_enforcer_actors,
};
